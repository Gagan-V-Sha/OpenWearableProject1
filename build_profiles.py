...
def build_user_profiles(user_id: str, group: pd.DataFrame) -> pd.DataFrame:
    ...
    steps_avg = roll["steps"].mean()          # mean per worn day
    active_avg = roll["active_minutes"].mean()  # mean per worn day
    ...
    # Missing-HRV indicator  
    prof["rmssd_missing"] = prof["rmssd_avg_7d"].isna().astype(int)

    # Ground-truth label for the window end date
    prof["recommendation"] = [
